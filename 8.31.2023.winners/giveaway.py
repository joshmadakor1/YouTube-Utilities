...
SECRET_WORDS = [
    ':' # This is to select comments with a time stamp, very ghetto
]

# Replace with the video ID of your target video: https://www.youtube.com/watch?v=yVEjs_9CyU4
VIDEO_ID = 'hUeHOtSLM-0'

# These are the headers for the CSV files containing the potential winners
CSV_HEADER = ['author_display_name', 'author_channel_url', 'comment',
              'comment_link', 'comment_id', 'comment_date', 'video_id']
...
def collect_comments_from_youtube_video(api_key, video_id):
    ...
    for comment in comments:
        count += 1
        print(
            f"Adding comment {count}: {comment['snippet']['topLevelComment']['snippet']['textDisplay']}")
        ...
        if 'replies' in comment:
            for reply in comment['replies']:
                count += 1
                print(
                    f"Adding comment {count}: {reply['snippet']['textDisplay']}")
...
def record_winners_of_each_secret_word_to_csv(secret_words, comments_with_secret_words):
    for secret_word in secret_words:
        ...
        filename = f'_{secret_words.index(secret_word)}.tsv'
...
all_comments = collect_comments_from_youtube_video(
    api_key=YOUTUBE_API_KEY, video_id=VIDEO_ID)
...
comments_with_secret_words = collect_comments_containing_secret_words(
    secret_words=SECRET_WORDS, all_comments=all_comments)
...
record_winners_of_each_secret_word_to_csv(
    secret_words=SECRET_WORDS, comments_with_secret_words=comments_with_secret_words)
